import {
  ...
  export default function GradientBoostingResult({ result, error, targetColumn }) {
    ...
    // Get Gradient Boosting configuration info
    const getGradientBoostingInfo = () => {
      if (!result) return [];
      ...
      return [
        ...
        { name: "Subsample", value: result.subsample || "N/A" },
        { name: "Min Samples Split", value: result.min_samples_split || "2" },
        { name: "Min Samples Leaf", value: result.min_samples_leaf || "1" },
        { name: "Max Features", value: result.max_features || "1.0" },
      ];
    };
    ...
    // Get training loss progression (simulated for Gradient Boosting)
    const getLossProgression = () => {
      if (!result?.n_estimators) return [];
  
      const data = [];
      const baseLoss = 1 - (result.accuracy || 0.7);
      
      for (let i = 1; i <= Math.min(result.n_estimators, 20); i++) {
        // Simulated loss reduction as more trees are added
        const lossReduction = baseLoss * Math.exp(-0.15 * i);
        data.push({
          estimator: i,
          loss: (baseLoss - lossReduction) * 100,
          cumulative: (baseLoss - lossReduction) * 100,
        });
      }
    ...
    const gradientBoostingInfo = getGradientBoostingInfo();
    const predictionComparison = getPredictionComparison();
    const lossProgression = getLossProgression();
        ...
        <div className="space-y-6">
          ...
          {result && (
            <>
              <div className="flex items-center gap-2 text-gray-900 dark:text-white">
                ...
                <h3 className="text-xl font-semibold">Gradient Boosting Model Performance</h3>
              </div>
  
              {/* Key Metrics */}
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="p-4 rounded-lg border border-gray-200 dark:border-zinc-800 bg-gradient-to-r from-green-50 to-teal-50 dark:from-green-900/10 dark:to-teal-900/10">
                ...
                <div className="p-4 rounded-lg border border-gray-200 dark:border-zinc-800 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/10 dark:to-indigo-900/10">
                  <div className="text-sm text-gray-600 dark:text-zinc-400 mb-1">
                    Precision
                  </div>
                ...
                <div className="p-4 rounded-lg border border-gray-200 dark:border-zinc-800 bg-gradient-to-r from-purple-50 to-pink-50 dark:from-purple-900/10 dark:to-pink-900/10">
                  <div className="text-sm text-gray-600 dark:text-zinc-400 mb-1">
                    Recall
                  </div>
                ...
                <div className="p-4 rounded-lg border border-gray-200 dark:border-zinc-800 bg-gradient-to-r from-orange-50 to-amber-50 dark:from-orange-900/10 dark:to-amber-900/10">
                  <div className="text-sm text-gray-600 dark:text-zinc-400 mb-1">
                    F1-Score
                  </div>
              ...
              <div className="mt-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  {gradientBoostingInfo.map((item, index) => (
                    <div
                      key={index}
                      className="p-4 rounded-lg border border-gray-200 dark:border-zinc-800 bg-gradient-to-r from-green-50 to-teal-50 dark:from-green-900/10 dark:to-teal-900/10"
                    >
                      <div className="text-sm text-gray-600 dark:text-zinc-400 mb-1">
                        {item.name}
                      </div>
                      ...
                      {item.name === "Learning Rate" && (
                        <div className="text-xs text-gray-500 dark:text-zinc-500 mt-1">
                          {result.learning_rate < 0.1 
                            ...
                            : result.learning_rate > 0.3
                            ? "High learning rate - fast learning"
              ...
              {lossProgression.length > 0 && (
                <div className="mt-6">
                  ...
                  <ResponsiveContainer width="100%" height={300}>
                    ...
                    >
                      ...
                      <XAxis
                        ...
                        stroke="#6b7280"
                        className="dark:stroke-zinc-400"
                        label={{ value: 'Number of Trees', position: 'insideBottom', offset: -5 }}
                      />
                      <YAxis
                        stroke="#6b7280"
                        className="dark:stroke-zinc-400"
                  ...
                  <p className="text-xs text-gray-500 dark:text-zinc-400 mt-2">
                    Shows how training loss decreases as more trees are added (simulated)
                  </p>
                </div>
              )}
              ...
              <div className="mt-6 p-4 rounded-lg border border-gray-200 dark:border-zinc-800 bg-gradient-to-r from-teal-50 to-green-50 dark:from-teal-900/20 dark:to-green-900/20">
                ...
                <div className="text-sm text-gray-600 dark:text-zinc-400 space-y-2">
                  <p>
                    ...
                    {result.accuracy > 0.9
                      ...
                      : "Poor performance - consider adjusting hyperparameters"}
                  </p>
                  <p>
                    <strong>Gradient Boosting Characteristics:</strong>{" "}
                    Builds trees sequentially where each tree corrects errors of previous ones.
                    Known for high predictive power but requires careful tuning.
                  </p>
                  <p>
                    <strong>Learning Rate {result.learning_rate}:</strong>{" "}
                    {result.learning_rate < 0.1
                      ? "Low learning rate - requires more trees for good performance"
                      : result.learning_rate > 0.3
                      ? "High learning rate - may overshoot optimal solution"
                      : "Moderate learning rate - good balance between speed and accuracy"}
                  </p>
                  <p>
                    <strong>Number of Estimators ({result.n_estimators}):</strong>{" "}
                    {result.n_estimators > 200
                      ? "Large ensemble - powerful but computationally expensive"
                      : result.n_estimators > 100
                      ? "Medium ensemble - good balance"
                      : "Small ensemble - may underfit complex patterns"}
                  </p>
                  <p>
                    <strong>Max Depth ({result.max_depth}):</strong>{" "}
                    {result.max_depth > 5
                      ? "Deep trees - captures complex patterns but may overfit"
                      : result.max_depth < 3
                      ? "Shallow trees - faster training but may underfit"
                      : "Moderate depth - balanced approach"}
                  </p>
                  {result.model_id && (
          ...
          {!result && !error && (
            <div className="text-center py-12 text-gray-500 dark:text-zinc-400">
              ...
              <p>
                Upload your data and train the Gradient Boosting classifier to see
                performance metrics
              </p>
            </div>
          )}
